...
const SpaceInvadersLayout = ({ children, href, className }: { children: React.ReactNode, href?: string, className?: string }) => {
    return (
        <div className={cn("flex flex-col justify-center items-center h-screen w-full bg-gray-200 dark:bg-surface-a10 sm:p-20 font-alien-robot", className)}>
            ...
            <div className="max-w-7xl p-4 w-full h-full grid grid-cols-12 gap-4 grid-rows-12 border-6 bg-[#b7bef1] border-[#400e63] text-[#400e63]" style={{ borderStyle: 'ridge' }}>
                {children}
            </div>
        </div>
    )
